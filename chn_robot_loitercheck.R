...
data_dir = paste0(path_pat, 'project_data/ASV_data/analysis_ready/CHN/')
...
#subset for CHN
CHN <- ME[ME$GNIS_Name == 'China Lake',]
...
CHN_reproj <- st_transform(CHN, crs = 'EPSG:32619')
#check projection
st_crs(CHN_reproj)
...
for(i in 1:length(robo_runs)) {
  #read in robot data
  CHN_robot_data <- read.csv(file.path(data_dir, robo_runs[i]))
  #make into simple feature; crs is WGS84
  CHN_robot_georef <- st_as_sf(CHN_robot_data, coords = c('longitude_gps_deg', 'latitude_gps_deg'), crs = 'EPSG:4326')
  #reproject to WGS UTM19N
  CHN_robot_georef_reproj <- st_transform(CHN_robot_georef, 'EPSG:32619')
  #check projection
  st_crs(CHN_robot_georef_reproj)

  #get the extent of the robot run for focus
  bbox_robot = st_bbox(CHN_robot_georef_reproj)

  #make CHN basemeap from robot extent
  CHN_map = tm_shape(CHN_reproj, bbox = bbox_robot) +
    tm_borders()

  #add a layer to check projection and loiter locs
  CHN_map_robot = CHN_map +
    tm_shape(CHN_robot_georef_reproj) +
  ...
  filename = paste0(CHN_robot_data$lake[1], '_', CHN_robot_data$date[1], '_', CHN_robot_data$deployment_instance[1], '.png')
  tmap_save(CHN_map_robot, file.path(map_dir, filename))
}
